...
router.get("/setup-status", async (req, res) => {
  try {
    const user = await User.findById(req.userId).select("courses topics firstName lastName school");
    res.json({
...
router.put("/profile", async (req, res) => {
  try {
    const { firstName, lastName, school, skills, bio, picture, certificates } = req.body;
    if (!school || !firstName || !lastName) {
      return res.status(400).json({ message: "firstName, lastName, school required" });
    }
    const update = {
      firstName, 
      lastName, 
    ...
    if (skills) update.skills = Array.isArray(skills) ? skills : skills.split(',').map(s => s.trim()).filter(s => s);
    if (bio !== undefined) update.bio = bio;
    if (picture !== undefined) update.picture = picture;
    if (certificates) update.certificates = Array.isArray(certificates) ? certificates : [];
